...
fn register_demo_node_types(reg: &mut NodeTypeRegistry) {
    ...
    reg.register(NodeTypeDefinition {
        type_id: "noise_texture".into(),
        display_name: "Noise Texture".into(),
...
pub async fn main() {
    ...
    // Connect: Math Add Result -> Color Mix Factor
    {
        ...
        let math_out = math_ports
            .iter()
            .find(|&&p| graph.world.get::<PortDirection>(p) == Some(&PortDirection::Output))
            .copied();
        let mix_fac = mix_ports
            .iter()
            .filter(|&&p| graph.world.get::<PortDirection>(p) == Some(&PortDirection::Input))
            .nth(2)
            .copied(); // Factor is the 3rd input
        drop(editor);
    ...
    // Connect: Noise Color -> Principled Base Color
    {
        ...
        let noise_color = noise_ports
            .iter()
            .find(|&&p| graph.world.get::<PortDirection>(p) == Some(&PortDirection::Output))
            .copied();
        let shader_base = shader_ports
            .iter()
            .find(|&&p| graph.world.get::<PortDirection>(p) == Some(&PortDirection::Input))
            .copied();
        drop(editor);
    ...
    // Connect: Principled BSDF -> Material Output Surface
    {
        ...
        let bsdf = shader_ports
            .iter()
            .find(|&&p| graph.world.get::<PortDirection>(p) == Some(&PortDirection::Output))
            .copied();
        let surface = out_ports
            .iter()
            .find(|&&p| graph.world.get::<PortDirection>(p) == Some(&PortDirection::Input))
            .copied();
        drop(editor);
